...
static void test_apdu_parser(void **state) {
    ...
    cmd.input_len = sizeof(apdu_bad_min_len);

    memset(&cmd, 0, sizeof(cmd));
    assert_false(apdu_parser(&cmd, apdu_bad_min_len));

    memset(&cmd, 0, sizeof(cmd));
    assert_false(apdu_parser(&cmd, apdu_bad_lc));

    memset(&cmd, 0, sizeof(cmd));
    cmd.input_len = sizeof(apdu);
    assert_true(apdu_parser(&cmd, apdu));
